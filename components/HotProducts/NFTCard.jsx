...
function NFTCard({ id, img, title, price, likes, sale, owner }) {
  ...
  console.log(ethers.parseEther("20").toString())
  ...
  const buyProperty = async (id) => {
    ...
    if (owner != address) {
      ...
      if (isConnected) {
        ...
        // const tx = await contract.listProperty(id, price)
        // const tx = await contract.buyProperty(id, { value: (ethers.parseEther(price.toString()).toString()) })
        const tx = await contract.getListing(id);
    ...
    <>
      <div className='flex group flex-col space-y-10 rounded-lg overflow-hidden border border-slate-400/10 pb-8 hover:shadow-xl duration-500 ease-in-out hover:shadow-white/5 relative'>
        ...
        <div className='px-6 flex flex-col space-y-3'>
          {/* Title */}
          <h1>{title}</h1>
          <h1>{owner.substring(0, 6) + " . . . . " + owner.substring(owner.length - 6, owner.length)}</h1>
          {/* Price & Like */}
        ...
        <div className='absolute hidden top-1/4 left-1/3 md:left-1/4 group-hover:flex animate-bounce transition-all ease-in-out duration-1000'>
          <button className='text-sm px-6 py-2 bg-indigo-600 rounded-md hover:bg-indigo-700 duration-200 ease-in-out' onClick={() => { buyProperty(id) }}>
            Buy Property
          </button>
        </div>
      </div>
